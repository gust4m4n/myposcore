...
func (s *CategoryService) CreateCategory(tenantID uint, name, description string) (*models.Category, error) {
...
func (s *CategoryService) UpdateCategory(categoryID, tenantID uint, name, description *string, isActive *bool) (*models.Category, error) {
	var category models.Category
	if err := s.db.Where("id = ? AND tenant_id = ?", categoryID, tenantID).First(&category).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, errors.New("category not found")
...
func (s *CategoryService) DeleteCategory(categoryID, tenantID uint) error {
	var category models.Category
	if err := s.db.Where("id = ? AND tenant_id = ?", categoryID, tenantID).First(&category).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
